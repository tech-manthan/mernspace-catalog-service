    ...
    priceConfiguration: {
      ...
      custom: {
        options: (value: string) => {
          ...
          try {
            const parsed = JSON.parse(value) as ProductPriceConfiguration;

            if (Object.keys(parsed).length === 0) {
              throw new Error("priceConfiguration cannot be empty");
            }
          } catch {
            throw new Error("priceConfiguration must be a Object");
          }
      ...
      customSanitizer: {
        options: (value: string) => {
          return JSON.parse(value) as ProductPriceConfiguration;
        },
      },
    },
    ...
    attributes: {
      ...
      custom: {
        options: (value: string) => {
          ...
          try {
            const parsed = JSON.parse(value) as ProductAttribute[];
            if (parsed.length === 0) {
      ...
      customSanitizer: {
        options: (value: string) => JSON.parse(value) as ProductAttribute[],
      },
    },
    "attributes.*.name": {
      exists: {
